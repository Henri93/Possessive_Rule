...
def isVerb(word):
	...
	if pos_l and 'v' in pos_l:
		if (max(pos_l, key=pos_l.get) == 'v' or pos_l['v'] >= 4):
			return True
...
for child in children:
	...
	ax = sns.lineplot(x="time", y="error_rate", data=df, markers=True)
	ax.set(ylim=(0.5, 1.05))
	ax.set_title(child)
	plt.xticks(rotation=-80)
	# ax.axvspan(productive_cords[0], productive_cords[-1], alpha=0.5, color='red')
	plt.show()
